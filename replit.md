...
-   **BCrypt Password Hashing**: `SecurityConfig.java` configures `BCryptPasswordEncoder` for secure password hashing, including auto-upgrade for legacy passwords.

**Feature Specifications:**
-   **Authentication System**: Login/register with email and password, session-based authentication.
-   **User Management**: Registration with skill levels.
-   **Match Management**: Create, join, leave matches; automatic confirmation at 4 players; match deletion if creator leaves; only creator can finish matches.
-   **Feedback System**: Players rate others post-match, updating perceived skill levels.
-   **Dynamic Sorting**: Matches can be sorted by date, popularity, or required level.
-   **Notifications**: Implemented via the Observer Pattern for match confirmations and completions.
-   **Data Seeding**: Automatic seeding of a complete dataset for testing.

**System Design Choices:**
-   **Observer Pattern**: `MatchConfirmedEvent` and `MatchFinishedEvent` published by `ApplicationEventPublisher` and handled by `MatchEventListener`.
-   **Strategy Pattern**: `MatchSortingStrategy` interface with concrete implementations (`DateSortingStrategy`, `PopularitySortingStrategy`, `LevelSortingStrategy`) dynamically selected.
-   **Singleton Pattern**: Leveraged through Spring's IoC container for service beans and `ApplicationEventPublisher`.
-   **JPA Entities**: `User`, `Match`, `Registration`, `Feedback` with appropriate relationships.
-   **Transactional Management**: `@Transactional` annotation for ACID properties and automatic rollbacks.
-   **Testing**: JUnit 5, Spring Boot Test, and JaCoCo for coverage analysis, focusing on scenario-based integration tests. Two application profiles (development and production) for logging management.
